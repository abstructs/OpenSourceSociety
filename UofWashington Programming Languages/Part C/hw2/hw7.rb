...
class Point < GeometryValue
  ...
  def intersectWithSegmentAsLineResult ls
    if inbetween(@x, ls.x1, ls.x2) && inbetween(@y, ls.y1, ls.y2)
      Point.new(@x, @y)
    else
      NoPoints.new
    end
...
class VerticalLine < GeometryValue
  ...
  def intersect something
    something.intersectVerticalLine self
  end
  def intersectPoint p
...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult ls
    ...
    if real_close(seg.x1, seg.x2)
      ...
      if real_close(seg.y2, seg2.y1)
        return Point.new(seg.x2, seg.y2)
      elsif seg.y2 < seg2.y1
        return NoPoints.new
      elsif seg.y2 > seg2.y2
        return LineSegment.new(seg2.x1, seg2.y1, seg2.x2, seg2.y2)
      else
        return LineSegment.new(seg2.x1, seg2.y1, seg.x2, seg.y2)
      end
    else
      if seg.x1 < seg2.x1
        (seg, seg2) = seg2, seg
      end
      if real_close(seg.x2, seg2.x1)
         return Point.new(seg.x2, seg.y2)
      elsif seg.x2 < seg2.x1
        return NoPoints.new
      elsif seg.x2 > seg2.x2
        return LineSegment.new(seg2.x1, seg2.y1, seg2.x2, seg2.y2)
      else
        return LineSegment.new(seg.x1, seg.y1, seg.x2, seg.y2)
      end
    end
  end
end
